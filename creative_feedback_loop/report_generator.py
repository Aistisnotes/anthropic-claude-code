...
def build_report_html(
    ...
    for rank, ins in enumerate(top_insights, 1):
        t = _escape(ins.get("title", ""))
        r = ins.get("avg_roas")
        roas_str = f" \u2014 {r:.2f}x ROAS" if r else ""
        top5_html += f"""
        <div style="background:#1e1e2e; border-left:4px solid #f59e0b; padding:10px 16px;
                    margin-bottom:8px; border-radius:0 8px 8px 0;">
          <span style="color:#f59e0b; font-weight:700; font-size:14px;">#{rank}</span>
          <span style="color:#fafafa; font-size:14px;"> {t}{roas_str}</span>
        </div>"""

    # ── Learnings ──────────────────────────────────────────────────────────────
    learnings_html = ""
    for item in pattern_results.get("learnings", []):
        learnings_html += f'<div class="learning-card">\u2022 {_escape(item)}</div>'

    # ── Hypotheses ─────────────────────────────────────────────────────────────
    hyp_html = ""
    for item in pattern_results.get("hypotheses", []):
        hyp_html += f'<div class="hyp-card">\u2022 {_escape(item)}</div>'
    ...
    def _dim_table(dims: list) -> str:
        ...
        for dim in dims:
            ...
            for j, v in enumerate(values):
                bg = "#1e1e2e" if j % 2 == 0 else "#262730"
                roas = v.get("avg_roas", 0)
                rc = "#27ae60" if roas >= 1.0 else "#e74c3c" if roas > 0 else "#888"
                wp = v.get("winner_pct", 0)
                lp = v.get("loser_pct", 0)
                wc = "#27ae60" if wp > lp else "#e74c3c" if wp < lp else "#888"
                rows += f"""
                <tr style="background:{bg};">
                  <td style="padding:7px 10px;color:#fafafa;font-size:12px;">{_escape(str(v.get("value","")))} </td>
                  <td style="padding:7px 10px;color:#ccc;text-align:right;font-size:12px;">{v.get("pct_all",0):.1f}%</td>
                  <td style="padding:7px 10px;color:{wc};text-align:right;font-size:12px;">{wp:.0f}%</td>
                  <td style="padding:7px 10px;color:#e74c3c;text-align:right;font-size:12px;">{lp:.0f}%</td>
                  <td style="padding:7px 10px;color:{rc};text-align:right;font-size:12px;">{roas:.2f}x</td>
                  <td style="padding:7px 10px;color:#fafafa;text-align:right;font-size:12px;">{_fmt_spend(v.get("spend",0))}</td>
                </tr>"""
            out += f"""
            <div style="margin-bottom:20px;">
              <h4 style="color:#fafafa;margin:0 0 6px 0;font-size:13px;">{name}</h4>
              <table style="width:100%;border-collapse:collapse;border-radius:6px;overflow:hidden;">
                <thead><tr style="background:#2d2d3d;">
                  <th style="padding:7px 10px;color:#fafafa;text-align:left;font-size:11px;">Value</th>
                  <th style="padding:7px 10px;color:#fafafa;text-align:right;font-size:11px;">% All</th>
                  <th style="padding:7px 10px;color:#fafafa;text-align:right;font-size:11px;">% Win</th>
                  <th style="padding:7px 10px;color:#fafafa;text-align:right;font-size:11px;">% Lose</th>
                  <th style="padding:7px 10px;color:#fafafa;text-align:right;font-size:11px;">ROAS</th>
                  <th style="padding:7px 10px;color:#fafafa;text-align:right;font-size:11px;">Spend</th>
                </tr></thead>
                <tbody>{rows}</tbody>
              </table>
            </div>"""
    ...
    dash_html = _dim_table(dashboard_data.get("dimensions", []))
    top50_html = _dim_table(top50_dashboard_data.get("dimensions", [])) if top50_dashboard_data else ""
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ background: #0e1117; color: #fafafa; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; padding: 28px; }}
  h1 {{ font-size: 26px; margin-bottom: 4px; }}
  h2 {{ font-size: 18px; color: #fafafa; margin: 22px 0 10px 0; }}
  .subtitle {{ color: #888; font-size: 13px; margin-bottom: 24px; }}
  .metrics {{ display: flex; gap: 10px; margin-bottom: 20px; }}
  .metric {{ background: #1e1e2e; border-radius: 8px; padding: 14px; flex: 1; text-align: center; }}
  .metric .value {{ font-size: 24px; font-weight: 700; color: #fafafa; }}
  .metric .label {{ font-size: 11px; color: #888; margin-top: 4px; }}
  .card {{ background: #1e1e2e; border-radius: 10px; padding: 16px; border: 1px solid #333; }}
  .badge {{ background: #e91e8c; color: white; padding: 3px 10px; border-radius: 5px; font-size: 12px; font-weight: 600; }}
  .exec-summary {{ background: #1e1e2e; border-left: 3px solid #e91e8c; padding: 14px; border-radius: 0 8px 8px 0; margin-bottom: 20px; color: #fafafa; font-size: 13px; line-height: 1.6; }}
  .learning-card {{ background: #1e1e2e; border-left: 3px solid #3b82f6; padding: 10px 14px; margin-bottom: 7px; border-radius: 0 8px 8px 0; color: #ccc; font-size: 13px; }}
  .hyp-card {{ background: #1e1e2e; border-left: 3px solid #f59e0b; padding: 10px 14px; margin-bottom: 7px; border-radius: 0 8px 8px 0; color: #ccc; font-size: 13px; }}
  .divider {{ border: none; border-top: 1px solid #333; margin: 20px 0; }}
</style>
</head>
<body>
  <h1>Creative Feedback Loop Report</h1>
  <div class="subtitle">{_escape(brand_name)} &middot; {_escape(date_range)} &middot; Generated {generated}</div>

  <div class="metrics">
    <div class="metric"><div class="value">{total_ads}</div><div class="label">Total Ads</div></div>
    <div class="metric"><div class="value" style="color:#27ae60;">{winners_count}</div><div class="label">Winners</div></div>
    <div class="metric"><div class="value" style="color:#e74c3c;">{losers_count}</div><div class="label">Losers</div></div>
    <div class="metric"><div class="value" style="color:#888;">{untested_count}</div><div class="label">Untested</div></div>
    <div class="metric"><div class="value">{_fmt_spend(total_spend)}</div><div class="label">Total Spend</div></div>
  </div>

  {"<div class='exec-summary'>" + exec_summary + "</div>" if exec_summary else ""}

  <h2>&#128202; Dimension Analysis (Classified Ads)</h2>
  {dash_html}

  {('<hr class="divider"><h2>&#128202; Top 50 by Spend</h2>' + top50_html) if top50_html else ""}

  <hr class="divider">
  <h2>&#128269; Pattern Insights</h2>
  {insights_html or "<p style='color:#888;'>No insights generated.</p>"}

  {('<hr class="divider"><h2>&#127942; Top 5 Strongest Patterns</h2>' + top5_html) if top5_html else ""}
...
def save_pdf_report(html: str, brand_name: str) -> str:
    """Save HTML as PDF to creative_feedback_loop/output/reports/. Returns file path."""
    import nest_asyncio
    nest_asyncio.apply()

    out_dir = Path(__file__).parent / "output" / "reports"
    out_dir.mkdir(parents=True, exist_ok=True)

    slug = brand_name.lower().replace(" ", "_") if brand_name else "report"
    filename = f"{slug}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    output_path = str(out_dir / filename)

    asyncio.get_event_loop().run_until_complete(generate_pdf(html, output_path))
    return output_path
