...
def market(
    ...
    # Handle blue ocean
    if result.competition_level == "blue_ocean" and result.blue_ocean_result:
        console.print("\n[bold yellow]🌊 Blue ocean report generated.[/]")
        console.print(
            "[dim]Run 'meta-ads compare' to generate the PDF if needed, "
            "or find the PDF in ~/Desktop/reports/[/]"
        )
...
def compare(
    ...
    elif result.competition_level == "blue_ocean" and result.blue_ocean_result:
        # Blue ocean — PDF was already generated by market pipeline, but regenerate if not found
        console.print(f"\n[bold yellow]🌊 Blue Ocean Market: {query}[/]")
        console.print(
            f"[dim]No brands have 50+ qualifying ads. "
            f"Blue ocean strategy generated.[/]"
        )
        _generate_blue_ocean_pdf_from_market(result.blue_ocean_result, query, config)
    else:
...
def _generate_blue_ocean_pdf_from_market(blue_ocean_result_dict: dict, keyword: str, config: dict) -> None:
    """Generate PDF from blue ocean result dict (from compare command)."""
    ...
    existing = list(pdf_out_dir.glob(f"{kw_slug[:20]}*blue_ocean*.pdf"))
    if existing:
        console.print(f"[dim]Blue ocean PDF already exists: {existing[0]}[/]")
        return

    try:
        bo = BlueOceanResult(**blue_ocean_result_dict)
        pdf_path = generate_blue_ocean_pdf_sync(
            blue_ocean_doc=bo,
            output_dir=pdf_out_dir,
        )
        console.print(f"[bold green]✓[/] Blue ocean PDF saved: {pdf_path}")
    except Exception as e:
        console.print(f"[yellow]⚠ Blue ocean PDF generation failed: {e}[/]")
